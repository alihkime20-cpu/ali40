...
def is_owner(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    return bool(update.effective_user and update.effective_user.id == context.application.bot_data["settings"].admin_user_id)

def admin_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("📊 الإحصائيات", callback_data="admin_stats"), InlineKeyboardButton("📢 القناة", callback_data="admin_channel")], [InlineKeyboardButton("ℹ️ التعليمات", callback_data="admin_help")]])
